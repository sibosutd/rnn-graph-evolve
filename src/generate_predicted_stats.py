...
diam_vec_gt = np.zeros((n_timestamps))
diam_vec_pred = np.zeros((n_timestamps))
...
for i in np.arange(1, n_t_train):
    ...
    # construct ground truth graph over time
    g_gt = nx.Graph(graph_matrix_train)
    largest_cc = max(nx.connected_component_subgraphs(g_gt), key=len)
    diam_vec_gt[i] = nx.diameter(largest_cc)
    diam_vec_pred[i] = nx.diameter(largest_cc)
...
for i in np.arange(n_t_train, n_timestamps):
    ...
    model.fit(pred_seq[:, [i-1], :],
              ...
              nb_epoch=1,
              verbose=0)
    g_pred = nx.Graph(graph_matrix)
    g_gt = nx.Graph(graph_matrix_train)
    largest_cc_pred = max(nx.connected_component_subgraphs(g_pred), key=len)
    largest_cc_gt = max(nx.connected_component_subgraphs(g_gt), key=len)
    diam_vec_pred[i] = nx.diameter(largest_cc_pred)
    diam_vec_gt[i] = nx.diameter(largest_cc_gt)
    print 'predict the {} edge: {}'.format(i, select_nodes)

plt.plot(np.arange(n_t_train),
         diam_vec_gt[:n_t_train], 'k', linewidth=2)
